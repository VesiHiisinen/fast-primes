/**
 * gpu-sieve-worker.mjs
 *
 * Standalone GPU sieve worker process (ESM, no TypeScript compilation).
 *
 * Usage: node dist/gpu/gpu-sieve-worker.mjs <startOdd> <endOdd> <numSegments>
 *
 * This .mjs file bypasses the SIGSEGV crash that occurs in the compiled
 * TypeScript worker when buffer sizes exceed ~640KB (numSegments > 5).
 * The crash appears to be related to how the compiled ESM bootstrap
 * interacts with dawn.node's internal allocator, not a fundamental
...
// Install dawn.node globals (GPUDevice, GPUBuffer, GPUBufferUsage, etc.)
...
function cpuSmallPrimeSieve(limit) {
...
async function main() {
  ...
  if (adapter === null) {
    process.stderr.write('No WebGPU adapter found\n');
    process.exit(1);
  }
  ...
  // Write device name to stderr so parent can read it
  process.stderr.write(`DEVICE:${adapterInfo.device}\n`);
